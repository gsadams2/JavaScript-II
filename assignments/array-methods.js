// A local community center is holding a fund raising 5k fun run and has invited 50 small businesses to make
///a small donation on their behalf for some much needed updates to their facilities.  Each business has
///assigned a representative to attend the event along with a small donation.
...
let fullName = [];
runners.forEach(function(namesYo) {
  fullName.push(namesYo.first_name);
  fullName.push(namesYo.last_name);
});
...
let allCaps = runners.map(function(capsYo) {
  return capsYo.first_name.toUpperCase();
});
console.log(allCaps);
...
let largeShirts = runners.filter(function(largeBoys) {
  if (largeBoys.shirt_size == "L") {
    return largeBoys.shirt_size;
  }
});
...
let ticketPriceTotal = runners.reduce(function(accumulator, donationsYo) {
  return accumulator + donationsYo.donation;
}, 0);
...
// Problem 1 - find average donation of each runner
